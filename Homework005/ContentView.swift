...
struct DrawView: UIViewRepresentable {
    func makeUIView(context: Context) -> UIView {
        ...
        view.layer.addSublayer(wtareProof())
        view.layer.addSublayer(stick())
//        view.layer.addSublayer(stickBall())
    ...
    func wtareProof () -> CAShapeLayer {
        let path = UIBezierPath()
        path.move(to: CGPoint(x: 126, y: 32))

        path.addQuadCurve(to: CGPoint(x: 35, y: 120), controlPoint:CGPoint(x: 52, y: 40))
        
        path.addQuadCurve(to: CGPoint(x: 77, y: 122), controlPoint:CGPoint(x: 60, y: 110))
        path.addQuadCurve(to: CGPoint(x: 126, y: 122), controlPoint:CGPoint(x: 100, y: 110))
        path.addQuadCurve(to: CGPoint(x: 175, y: 122), controlPoint:CGPoint(x: 150, y: 110))
        path.addQuadCurve(to: CGPoint(x: 212, y: 120), controlPoint:CGPoint(x: 195, y: 110))
        
        path.addQuadCurve(to: CGPoint(x: 126, y: 32), controlPoint:CGPoint(x: 200, y: 40))
        path.lineJoinStyle = .round //為何沒有反應？
        path.close()
    ...
    func stick () -> CAShapeLayer {
        ...
        path.lineCapStyle = .round //為何沒有反應？
...
//    func stickBall () -> CAShapeLayer {
//        let aDegree = CGFloat.pi / 180
//        let path = UIBezierPath(arcCenter: CGPoint(x: 94, y: 200), radius: 11.5, startAngle: aDegree * 0, endAngle: aDegree * 360, clockwise: true)
//
//        let layer = CAShapeLayer()
//        layer.path = path.cgPath
//        layer.fillColor = CGColor(red: 0, green: 0, blue: 1, alpha: 1)
